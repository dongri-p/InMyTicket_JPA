...
## 2. 기술 스택 (Tech Stack)
* **Backend Core:** Java 17, Spring Boot 3.5.14
...
* **해결방안**: **지연 로딩(LAZY)** 정책을 기본으로 유지하되, 다중 연관관계 조회가 필요한 시점에는 **페치 조인(Fetch Join)**을 적용하여 1번의 쿼리로 연관 데이터를 무결하게 통합 조회하도록 SQL을 최적화했습니다.
...
* **해결방안**: 대규모 동시 요청 속에서 완벽한 데이터 무결성을 확보하기 위해 데이터베이스 로우(Row) 수준에서 쓰기 잠금을 획득하는 **비관적 락(Pessimistic Lock)**을 도입했습니다.
* **검증**: 
